...
<div class="row">
    ...
    <h3><?php echo Yii::t('app', 'Załączniki'); ?></h3>
        ...
        <?php
        $user = Yii::$app->user;
        if ($user->can('gearAttachmentsCreate')) {
            echo Html::a(Yii::t('app', 'Dodaj'), ['gear-attachment/create', 'gearId' => $model->id], ['class' => 'btn btn-success btn-xs'])." ";
        }
            echo Html::a(Yii::t('app', 'Pobierz'), ['gear-attachment/download-all'], ['class' => 'btn btn-success btn-xs download-files-button'])." ";
            echo Html::a(Yii::t('app', 'Wyślij'), ['gear-attachment/send'], ['class' => 'btn btn-success btn-xs send-files-button']);
        ?>
        </div>
        <div class="col-md-7">
        <div class="file-manager">
                                ...
                                <?php foreach (\common\models\GearAttachemntType::find()->where(['active'=>1])->asArray()->all() as $typ){ ?> 
...
<div class="col-lg-12">
<?php foreach ($model->gearAttachments as $file){ ?>
...
<?php if(($file->extension=='jpg')||($file->extension=='png')||($file->extension=="gif"))
        ...
        <?php
        if ($user->can('gearAttachmentsCreate')) {
            echo Html::a("<i class='fa fa-pencil'></i>", ['gear-attachment/update', 'id' => $file->id], ['class' => "badge badge-primary pull-right confirm-bagde"]);
            echo Html::a("<i class='fa fa-trash'></i>", ['gear-attachment/delete', 'id' => $file->id], ['class' => "badge badge-danger pull-right delete-bagde", 'data' => [
