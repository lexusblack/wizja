...
<div class="row">
    <div class="col-md-12">
        ...
        if ($user->can('eventEventEditEyeOuterGearManage')) {
            echo Html::a(Yii::t('app', 'Zarządzaj'), ['outer-warehouse/assign', 'id' => $model->id, 'type' => 'rent'], ['class' => 'btn btn-success']);
        }
        ?>
...
<div class="row">
    ...
    <div class="panel_mid_blocks">
        ...
        <h5><?php echo Yii::t('app', 'Zapotrzebowanie na sprzęt zewnętrzny'); ?></h5>
                ...
                'columns' => [
                    ...
                    [
                        ...
                        'value' => function($gear) use ($model) {
                            $gear_no = $model->getRentOuterGearModels()->where(['outer_gear_model_id'=>$gear->id])->one();
                     ...
                     [
                        ...
                        'value' => function($gear) use ($model) {
                            ...
                            $gears = $model->getRentOuterGears()->where(['IN', 'outer_gear_id', $gear_id])->all();
                            $return = '';
                            foreach ($gears as $g)
                            {
                                $return .= $g->outerGear->company->name." - ".$g->quantity." ".Yii::t('app', 'szt').".<br/>";
                            }
                            $gear_one = $model->getRentOuterGearModels()->where(['outer_gear_model_id'=>$gear->id])->one();
