...
<!--        <button id="toggle_time_helper">TimeHelper</button>-->
        ...
        <div class="line-block">
            ...
            <?= Html::a("Zwiń/Rozwiń wszystkie", '#', ['id'=>'close-all-button', 'class'=>'btn btn-primary btn-xs'])?>
            <?php
            if (false){
                $display = 'none';
...
$this->registerCss("
    ...
    .vehicle_box { background-color: white; display: none; }
    ...
    .user_confirm_plantimeline{ border-left:5px solid #1ab394; }
...
<script>
    ...
    window.planboard_event_id = false;
    window.planboard_event = false;
    window.planboard_schedules = false;
    var user_types = null;
    var skills = null;
    ...
    var planboardApp = angular.module("planboardApp", ["ngSanitize"]).run(function($rootScope,$window,$timeout,restService) {
        ...
        $rootScope.planboard_event_id = $window.planboard_event_id;
        $rootScope.planboard_event = $window.planboard_event;
        if ($rootScope.planboard_event_id)
            $rootScope.planboard_schedules = $window.planboard_event.schedules;
        ...
        $rootScope.changeUserPanelById = function(item) {
            if (item!=false)
            {
                $window.planboard_event = item;
                $window.planboard_event_id = item.id;
            }else{
                $window.planboard_event = item;
                $window.planboard_event_id = item;                
            }
        ...
        (function checkWindow(){
            ...
            $rootScope.planboard_event = $window.planboard_event;
            if ($rootScope.planboard_event_id)
                $rootScope.planboard_schedules = $rootScope.planboard_event.schedules;
            else
                $rootScope.planboard_schedules = false;
    ...
    planboardApp.filter("getScheduleBG",function(){
        return function (data) {
            r = "";
            r += "<div style='background-color:"+data.color+"'>"+data.prefix+"</div>";
            return r;
        }
    });
    ...
    planboardApp.filter("getDateBG", function ($rootScope) {
        return function (data) {
            if ($rootScope.planboard_event_id)
            {
                ...
                var schedules = $rootScope.planboard_event.schedules;
            ...
            if (schedules.length)
                var one_size = 100/schedules.length;
            else
                var one_size = 100;
                ...
                for (var i=0; i<schedules.length; i++)
                {
                    var color = none_color;
                                 var s_start = Date.parse(schedules[i].start_time);
                                var s_end = Date.parse(schedules[i].end_time);                   
                    if(vacation !== undefined) {
                        vacation.forEach(function(element, index, array){
                                if(element.start_date === null || element.end_date === null){
                                    return "";
                                }
                                var e_start = Date.parse(element.start_date);
                                var e_end = Date.parse(element.end_date);
                                if ((e_start <s_end)&&(e_end>s_start))
                                {
                                    if (vacation.status==10)
                                    {
                                        color = vacation_s_confirmed;
                                    }else{
                                        color = vacation_s_not_confirmed;
                                    }
                                }
                        });
                    }
                    ...
                    workingTimes.forEach(function(element, index, array){
                        ...
                        var e_start = Date.parse(element.start_time);
                        var e_end = Date.parse(element.end_time);
                                if ((e_start<=s_start)&&(e_end>=s_end))
                                {
                                    color = event_color;
                                }else{
                                    if ((e_start<s_end)&&(e_end>s_start))
                                    {
                                        color = half_event_color;
                                    }
                                }                        
                    });
                    }
...
#w0-tab0 .weekdays_list{
    padding-right:16px;
}

#w0-tab1 .weekdays_list{
    padding-right:16px;
}

.planboard_drug_box .event_list li {
    display: block;
    float: left;
    text-align: center;
    font-weight: bold;
