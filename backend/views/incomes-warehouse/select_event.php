...
?>
    ...
    <label for="event_type" style="font-weight: normal;"> <?= Yii::t('app', 'Rodzaj imprezy') ?>:
        <select id="event_type" class="form-control">
            <option value="<?= OutcomesWarehouse::EVENT_TYPE_NONE ?>"><?= Yii::t('app', 'Żadne') ?></option>
    ...
    <div id='event_id_wrapper' style='display: <?= $display_event ?>;'>
        <?= Select2::widget(['name' => 'event_id', 'data' => $event_dropdown, 'id' => 'select_event_id', 'options' => ['placeholder' => 'Znajdź event',],]); ?>
    </div>

    <div id='rent_id_wrapper' style='display: <?= $display_rent ?>;'>
        <?= Select2::widget(['name' => 'rent_id', 'data' => $rent_dropdown, 'id' => 'select_rent_id', 'options' => ['placeholder' => 'Znajdź wypożyczenie',]]); ?>
    </div>

    <div id='customer_id_wrapper' style='display: <?= $display_customer ?>;'>
        <?= Select2::widget(['name' => 'customer_id', 'data' => $customer_dropdown, 'pluginOptions' => ['tags'], 'id' => 'select_customer_id', 'options' => ['placeholder' => 'Znajdź klienta',
...
$this->registerJs('
    ...
    $("#next").click(function(){
        ...
        if (event_type == ' . OutcomesWarehouse::EVENT_TYPE_NONE . ') {
            ...
            if (customer == "") {
                $.notify({message: "'.Yii::t('app', 'Nie wybrano klienta, dla którego chcesz przyjąć sprzęt').'",},{type: "danger"});
            }
            else {
        ...
        if (event_type == ' . OutcomesWarehouse::EVENT_TYPE_EVENT . ') {
            ...
            if (event == "") {
                $.notify({message: "'.Yii::t('app', 'Nie wybrano eventu, dla którego chcesz przyjąć sprzęt').'",},{type: "danger"});
            }
            else {
        ...
        if (event_type == ' . OutcomesWarehouse::EVENT_TYPE_RENT . ') {
            ...
            if (rent == "") {
                $.notify({message: "'.Yii::t('app', 'Nie wybrano wypożyczenia, dla którego chcesz przyjąć sprzęt').'",},{type: "danger"});
            }
            else {
