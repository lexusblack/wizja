...
$(function(){
    ...
    var options = {
        ...
        end: momentToDate(moment(window.date_start + " 00:00").add(7, 'days')),
        minHeight: 500,
        zoomMin: 1000*60*60*24*3, // 3 dni
            ...
            success: function (events) {
               // console.log(events);
                for (var i = 0; i < events.length; i++) {
                    loadedEvents[events[i].id] = events[i].id;
                    items.add({
                        id: events[i].id,
            ...
            $(this).contextMenu({
                ...
                menuSelected: function (invokedOn, selectedMenu, event_id, user_id, vehicle_id) {
                    if(selectedMenu.hasClass("open_vehicle_modal")){
                        openVehicleModal(event_id, vehicle_id, false, 0);
                    }
    ...
    // Open modal - vehicle
    function openVehicleModal(event_id, vehicle_id, update_event_vehicle_data, just_assigned){
        var modal = $("#vehicle_modal");
        modal.find(".modalContent").load("vehicle-form?event_id="+event_id+"&vehicle_id="+vehicle_id);
        modal.modal("show");
    }
            ...
            receive: function(ev, ui) {
                ...
                if(car_in_list_count > 1){
                    ...
                    assignVehicle(car_id,event_id,1,function() {
                        openVehicleModal(event_id, car_id);
                    });
                }
            },
            stop: function (event, ui) {
