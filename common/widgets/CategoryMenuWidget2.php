...
class CategoryMenuWidget2 extends Widget
{
    public $route = null;
    ...
    public function run()
    {
        ...
        {
            $mainCategories = CRCategory::find()->where(['id'=>Yii::$app->user->identity->gear_category_id])->all();
        }else{
...
//        $subCategories = CRCategory::find()
//            ->subcategories($this->_currentMainId)
//            ->orderBy(['name'=>SORT_ASC])
//            ->all();

        $cat = CRCategory::findOne($this->_currentMainId);
        $subCategories = [];
        if ($cat !== null) {
...
//            }
        ...
        //FIXME: rekurencja!!!
        $index = 1;
        $cat = CRCategory::findOne($this->_currentSubId);
        if ($cat !== null)
        {
            $subCategories = $cat->children(1)->andWhere(['active'=>1])->all();
            foreach ($subCategories as $category)
