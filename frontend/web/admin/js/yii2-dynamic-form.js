...
(function ($) {
    ...
    var _restoreSpecialJs = function(widgetOptions) {
        ...
        // "kartik-v/yii2-widget-DateRangePicker"
        var $hasDateRangePicker = $(widgetOptionsRoot.widgetItem).find('[data-krajee-DateRangePicker]');
        if ($hasDateRangePicker.length > 0) {
            $hasDateRangePicker.each(function() {
                var _this = $(this);
                _this.closest('.input-group').daterangepicker(window[_this.data("krajee-daterangepicker")], function(start,end,label){var val=start.format('YYYY-MM-DD HH:mm:ss') + ' - ' + end.format('YYYY-MM-DD HH:mm:ss'); _this.val(val).trigger('change');});
            });
        }
    };
